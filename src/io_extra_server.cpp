...
#include "external_io/setIO.h"
#include "external_io/setPWM.h"
...
void IO_cb(const external_io::setIO::ConstPtr msg) {
...
void pwm_cb(const external_io::setPWM::ConstPtr &msg) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber io_suber =
      nh.subscribe<external_io::setIO>("/setIO", 10, IO_cb);

  // PWM控制订阅者
  ros::Subscriber pwm_suber =
      nh.subscribe<external_io::setPWM>("/setPWM", 10, pwm_cb);
  ...
  ROS_INFO("  /setIO (external_io/setIO) - IO control");
  ROS_INFO("  /set_servo (std_msgs/Int8) - Servo control");
  ROS_INFO("  /setPWM (external_io/setPWM) - PWM control");
